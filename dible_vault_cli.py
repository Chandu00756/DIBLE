...
def status(as_json):
    ...
    data_count = len(list(DATA_DIR.glob("*.dible")))
    info = {
        "vault_version": "2.0.0",
        "algorithm": "AES-256-GCM",
        "kdf": "PBKDF2-SHA3-256 / 600k iters",
        "device_id": device_id[:16] + "...",
        "keys_stored": key_count,
        "encrypted_files": data_count,
    ...
    t = Table(title="Vault Status", box=box.ROUNDED, show_header=True, header_style="bold bright_blue")
...
# ──────────────────── keygen ────────────────────────────────────────────
@vault.command()
@click.option("--name", default="", help="Optional key name / label")
def keygen(name):
    ...
    console.print(Panel(f"[bright_green]Key generated[/bright_green]
"
...
@click.option("-o", "--output", "dst",  default="",    help="Output .dible file (default: <input>.dible)")
@click.option("-k", "--key-id", "kid",  default="",    help="Key ID to use (default: newest key)")
@click.option("-p", "--password",       default="",    help="Derive key from password instead of stored key")
def encrypt(src, dst, kid, password):
    """Encrypt a file. Use --key-id or --password."""
    engine = _engine()
    src_path = Path(src)
    ...
    if password:
        ...
        if not kid:
            ...
            if not keys:
                raise click.ClickException("No keys found. Run keygen first.")
            kid = json.loads(keys[0].read_text())["key_id"]
        key = _load_key(kid)
        kid_used = kid
        salt_hex = ""

    blob = engine.encrypt(src_path.read_bytes(), key)
    meta = json.dumps({"key_id": kid_used, "salt": salt_hex, "src_name": src_path.name}).encode()
    meta_len = len(meta).to_bytes(4, "big")
    dst_path.write_bytes(meta_len + meta + blob)
    dst_path.chmod(0o600)
    _audit("encrypt", f"{src} -> {dst_path}")
    console.print(f"[bright_green]Encrypted[/bright_green] {src_path.name} → [bright_cyan]{dst_path}[/bright_cyan]")
...
def decrypt(src, dst, password):
    ...
    console.print(f"[bright_green]Decrypted[/bright_green] {src_path.name} → [bright_cyan]{dst_path}[/bright_cyan]")

# ──────────────────── encrypt-text ──────────────────────────────────
@vault.command("encrypt-text")
@click.argument("text")
@click.option("-k", "--key-id", "kid", default="", help="Key ID (default: newest key)")
@click.option("-p", "--password",      default="", help="Derive key from password")
def encrypt_text(text, kid, password):
    """Encrypt a text string and print base64 ciphertext."""
    engine = _engine()
    if password:
        key, salt = engine.derive_key(password)
        salt_hex = salt.hex()
    ...
    out = {"ciphertext": _b64.b64encode(blob).decode(), "key_id": kid, "salt": salt_hex}
...
@click.option("-s", "--salt",   "salt_hex", default="", help="Salt hex (required with --password)")
def decrypt_text(ciphertext_b64, kid, password, salt_hex):
    """Decrypt base64 ciphertext and print plaintext."""
    engine = _engine()
    import base64 as _b64
    blob = _b64.b64decode(ciphertext_b64)
    if password:
        salt = bytes.fromhex(salt_hex) if salt_hex else b" " * 32
        key, _ = engine.derive_key(password, salt)
    else:
        key = _load_key(kid)
    pt = engine.decrypt(blob, key)
...
@click.option("--confirm", is_flag=True, required=True, help="Must pass --confirm to delete")
...
# ──────────────────── audit ─────────────────────────────────────────────
@vault.command()
@click.option("-n", "--lines", default=20, help="Last N audit entries to show")
def audit(lines):
    """Show the last N vault audit log entries."""
    log = LOGS_DIR / "audit.log"
    if not log.exists():
        console.print("[bright_yellow]No audit log found.[/bright_yellow]")
        return
...
@vault.command()
def test():
    """Run built-in round-trip verification."""
    import tempfile
    engine = _engine()
    errors = []
    ...
    recovered = engine.decrypt(ct, key_obj.raw_key)
    if recovered != pt:
        errors.append("raw-key round-trip FAILED")
    else:
        console.print("[bright_green]✓[/bright_green] Raw-key AES-256-GCM round-trip")

    # 2. password KDF round-trip
    passwd = secrets.token_urlsafe(16)
    key2, salt2 = engine.derive_key(passwd)
    ct2 = engine.encrypt(pt, key2)
    key2b, _ = engine.derive_key(passwd, salt2)
    recovered2 = engine.decrypt(ct2, key2b)
    if recovered2 != pt:
        errors.append("password-KDF round-trip FAILED")
    else:
    ...
    try:
        ...
        errors.append("tamper detection FAILED (no exception raised)")
    except Exception:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if dec.read_bytes() != pt:
            errors.append("file encrypt/decrypt round-trip FAILED")
        else:
